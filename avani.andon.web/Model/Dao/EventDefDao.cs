    ...
    public class EventDefDao
    {
        ...
        public bool Update(tblEventDef entity)
        {
            try
            {
                ...
                eventDef.Name_EN = entity.Name_EN;
                eventDef.Name_VN = entity.Name_VN;
        ...
        public List<tblEventDef> listAllByLineOnline()
        {
            //List<tblEventDef> model = db.tblEventDefs.ToList();

            return db.tblEventDefs.Where(p=>p.Id>2&&p.Id<7).OrderBy( x => x.zOrder).ToList();
        }
        public IEnumerable<tblEventDef> ListAllPaging(string searchString)
        {
            ...
            {
                model = model.Where(x => x.Name_VN.Contains(searchString) || x.Name_EN.Contains(searchString));
            }

            return model.OrderByDescending(x => x.Name_VN).OrderByDescending(x => x.Name_EN).ToList();
        }
